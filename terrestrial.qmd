---
title: "Terrestrial"
---

```{r, include=FALSE}
knitr::opts_chunk$set(echo = FALSE, message=FALSE, warning=FALSE)
```

```{r setup}
library(ggiraph)
library(patchwork)
library(tidyverse)
library(neon4cast)
library(score4cast)
library(thematic)
#library(bslib)
#bslib::theme_bootswatch("lux")
thematic_rmd(bg="white", fg="black", accent="blue")

source("R/plot-utils.R")

```

```{r}
dashboard <- yaml::read_yaml("dashboard.yml")
```

# Daily averages

```{r }
#combined <- combined_scores("terrestrial_daily") |> filter(time > dashboard$start_time)

theme <- "terrestrial_daily"
combined <- arrow::open_dataset("cache", partition=c("target_id", "year")) |>
  filter(target_id == theme,
         time >= lubridate::as_datetime(dashboard$start_time))  |>
  collect()
```

## Most recent forecasts

Below is the latest forecasts for which we have at least 15 observations. Mouse over to see the team id, scroll to zoom.

```{r}
day <- combined |>
  filter(!is.na(observed)) |>
  count(target_id, variable, model_id, start_time, site_id) |>
  filter(n > 15) |> 
  # at least 15 data points
  filter(start_time == max(start_time)) |> # must happen second
  select(start_time) |> distinct(start_time) |> first()
```


## Leaderboard (daily averages)

::: {.panel-tabset}

## Latent evaporation

```{r}
combined |> filter(start_time == day, variable == "le") |> forecast_plots()
```

## CO2 Flux

Net ecosystem exchange of CO2 (NEE) 


```{r}
combined |> filter(start_time == day, variable == "nee") |> forecast_plots()
```

## vswc 

No data

```{r}
combined |> filter(variable == "vswc", !is.na(observed))
```

:::


```{r}
filled_scores <- fill_scores(combined, null_model = "persistence")
```

```{r}
leaderboard <-  filled_scores |> 
  group_by(variable, model_id) |>
  summarise(crps = mean(crps),
            logs = mean(logs),
            percent_na = mean(is.na(crps_model)),
            .groups = "drop")
```

```{r}
by_start <-  filled_scores |> 
  group_by(variable, model_id, start_time, site_id) |>
  summarise(crps = mean(crps),
            logs = mean(logs),
            percent_na = mean(is.na(crps_model)),
            .groups = "drop")

```


::: {.panel-tabset}

## Latent evaporation

```{r}
leaderboard_plots(leaderboard, NULL, "le")
```

## CO2 flux

```{r}
leaderboard_plots(leaderboard, NULL, "nee")
```

## vswc

(no data)

:::

Overall team rankings by forecast skill. Lower values indicate better predictions. Both score probabilistic forecasts, but log skill penalizes observations outside expected range much more heavily. **Tip**: *Mouse over a color to highlight the scores of one a team*


----------------


# 30 Minute averages

```{r }

#combined <- combined_scores("terrestrial_30min") |>  filter(time > dashboard$start_time)

theme <- "terrestrial_30min"
combined <- arrow::open_dataset("cache", partition=c("target_id", "year")) |>
  filter(target_id == theme,
         time >= lubridate::as_datetime(dashboard$start_time)) |>
  collect()|>
  mutate(start_time = as.Date(start_time))

```


## Most recent forecasts

Below is the latest forecasts for which we have at least 15 observations. Mouse over to see the team id, scroll to zoom.

```{r}
day <- combined |>
  filter(!is.na(observed)) |>
  count(target_id, variable, model_id, start_time, site_id) |>
  filter(n > 100) |> 
  # at least 15 data points
  filter(start_time == max(start_time)) |> # must happen second
  select(start_time) |> distinct(start_time) |> first()

end <- as.Date(day) + 4
```



::: {.panel-tabset}

## Latent evaporation

```{r}
combined |> filter(start_time == day, variable == "le", time < end) |> forecast_plots(ncol=3)
```

## CO2 Flux

Net ecosystem exchange of CO2 (NEE) 


```{r}
combined |> filter(start_time == day, variable == "nee", time < end) |> forecast_plots()
```

## vswc 

No data

```{r}
combined |> filter(variable == "vswc", !is.na(observed))
```

:::


```{r}
## Fill scores relative to the "climatology" null


filled_scores <- fill_scores(combined, null_model = "climatology")
```

```{r}
leaderboard <-  filled_scores |> 
  group_by(variable, model_id) |>
  summarise(crps = mean(crps),
            logs = mean(logs),
            percent_na = mean(is.na(crps_model)),
            .groups = "drop")
```

```{r}
by_start <-  filled_scores |> 
  group_by(variable, model_id, start_time, site_id) |>
  summarise(crps = mean(crps),
            logs = mean(logs),
            percent_na = mean(is.na(crps_model)),
            .groups = "drop")

```


::: {.panel-tabset}

## Latent evaporation

```{r}
leaderboard_plots(leaderboard, NULL, "le")
```

## CO2 flux

```{r}
leaderboard_plots(leaderboard, NULL, "nee")
```

## vswc

(no data)

:::

Overall team rankings by forecast skill. Lower values indicate better predictions. Both score probabilistic forecasts, but log skill penalizes observations outside expected range much more heavily. **Tip**: *Mouse over a color to highlight the scores of one team*























