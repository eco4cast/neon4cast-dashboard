...
combined <- arrow::open_dataset("cache/terrestrial_daily") |>
...
combined <- arrow::open_dataset(glue("cache/{theme}")) |>
  filter(datetime >= lubridate::as_datetime(dashboard$reference_datetime)) |>
  collect()|>
...
```{r}
combined |> filter(reference_datetime == day, variable == "le", time < end) |> forecast_plots(ncol=3)
```
...
```{r}
combined |> filter(reference_datetime == day, variable == "nee", time < end) |> forecast_plots()
```
