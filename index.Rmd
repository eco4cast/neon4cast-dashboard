...
```{r}
p <-   combined %>% 
  filter(theme == "phenology", target == "gcc_90", horizon > 30) %>% 
  group_by(time, siteID, team) %>%
  slice_min(horizon) %>% 
  ggplot() +
  geom_ribbon(aes(x = time, ymin = lower95, ymax = upper95, fill = team), alpha = 0.2) +
  geom_line(aes(time, mean, col = team)) + geom_point(aes(time, obs), size = .1) + 
...
```{r}
p <- combined %>% 
  filter(theme == "phenology", target == "rcc_90", horizon > 30) %>% 
  group_by(time, siteID, team) %>%
  slice_min(horizon) %>% 
  ggplot() +
  geom_ribbon(aes(x = time, ymin = lower95, ymax = upper95, fill = team), alpha = 0.2) +
...
p <- combined %>% 
  filter(theme == "aquatics", horizon > 6) %>% 
  group_by(time, siteID, team) %>%
  slice_min(horizon) %>% 
  ggplot() +
  geom_ribbon(aes(x = time, ymin = lower95, ymax = upper95, fill = team), alpha = 0.2) +
...
p <- combined %>% 
  filter(theme ==  "terrestrial_daily", horizon > 30) %>% 
  group_by(time, siteID, team) %>%
  slice_min(horizon) %>% 
  ggplot() +
  geom_ribbon(aes(x = time, ymin = lower95, ymax = upper95, fill = team), alpha = 0.2) +
...
p <- combined %>% 
  filter(theme ==  "terrestrial_30min", horizon > 30, target == "le") %>% 
  group_by(time, siteID, team) %>%
  slice_min(horizon) %>% 
  ggplot() +
  geom_ribbon(aes(x = time, ymin = lower95, ymax = upper95, fill = team), alpha = 0.2) +
...
## Could consider displaying older ones
p <- combined %>% 
  filter(theme ==  "terrestrial_30min", horizon > 30, target == "nee") %>% 
  group_by(time, siteID, team) %>%
  slice_min(horizon) %>% 
  ggplot() +
