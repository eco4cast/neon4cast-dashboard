...
```{r}
combined <- combined_scores("beetles") |> filter(!is.na(observed))
...
```{r}
richness <- combined |> 
  filter(start_time == day, variable == "richness", time < end)
```
...
```{r}
abundance <- combined |> 
  filter(start_time == day, variable == "abundance", time < end)
```
