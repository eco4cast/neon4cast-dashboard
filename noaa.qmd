...
temp <- arrow::open_dataset(taat) |> 
  ...
  filter(startDateTime > cutoff, siteID %in% sites) |>
...
tmp_scores |> filter(sites %in% sites_subset)  |> forecast_plots()
```
...
rh_scores  |> filter(sites %in% sites_subset)  |> forecast_plots()
```
