...
combined <- arrow::open_dataset(s3$path("RH")) |> 
  ...
  mutate(reference_datetime = reference_datetime, target_id = "weather") |>
  collect()
```
...
richness <- combined |> filter(reference_datetime == day, variable == "RH")
```
...
horizon <-  combined |> filter(!is.na(crps)) |> 
  mutate(horizon = as.numeric(time - reference_datetime, units="hours")) |>
