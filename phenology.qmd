...
```{r}
green |> filter(pub_time == day) |> filter(site_id %in% sites[1:6]) |> plotme()
```

## 7-12

```{r}
green |> filter(pub_time == day, site_id %in% sites[7:12]) |> plotme()
```

## 13 - 18

```{r}
green |>  filter(pub_time == day, site_id %in% sites[13:18]) |>  plotme()
```
:::
