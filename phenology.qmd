...
day <- green |>
  ...
  count(target_id, variable, model_id, start_time) |>
...
```{r}
green |> filter(start_time == day) |> filter(site_id %in% sites[1:6]) |> plotme()
```

## 7-12

```{r}
green |> filter(start_time == day, site_id %in% sites[7:12]) |> plotme()
```

## 13 - 18

```{r}
green |>  filter(start_time == day, site_id %in% sites[13:18]) |>  plotme()
```
:::
...
board2_logs <- 
  by_start |> 
  filter(percent_na < .1) |>
  ggplot(aes(start_time, logs, col=model_id)) + 
  geom_line_interactive(aes(tooltip = model_id, data_id = model_id), size=1, show.legend = FALSE)  +
  labs(title = "Forecast skill")
...
greenness_forecasts <-
  combined |>
  filter(start_time==max(start_time, na.rm=TRUE), site_id %in% some_sites) |>
